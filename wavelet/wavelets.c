...
DiscreteWavelet* blank_discrete_wavelet(size_t filters_length)
{
    ...
    w = (DiscreteWavelet*)wtmalloc(sizeof(DiscreteWavelet));
    if (w == NULL) return NULL;
...
ContinuousWavelet* blank_continuous_wavelet(void)
{
    ...
    w = (ContinuousWavelet*)wtmalloc(sizeof(ContinuousWavelet));
    if (w == NULL) return NULL;
...
DiscreteWavelet* copy_discrete_wavelet(DiscreteWavelet* base)
{
    ...
    if (base->dec_len > 0)
    {
        w->dec_lo_float = (float*)wtmalloc(w->dec_len * sizeof(float));
        w->dec_hi_float = (float*)wtmalloc(w->dec_len * sizeof(float));
        w->dec_lo_double = (double*)wtmalloc(w->dec_len * sizeof(double));
        w->dec_hi_double = (double*)wtmalloc(w->dec_len * sizeof(double));
        if (w->dec_lo_float == NULL || w->dec_hi_float == NULL ||
            w->dec_lo_double == NULL || w->dec_hi_double == NULL) {
            free_discrete_wavelet(w);
            return NULL;
    ...
    if (base->rec_len > 0)
    {
        w->rec_lo_float = (float*)wtmalloc(w->rec_len * sizeof(float));
        w->rec_hi_float = (float*)wtmalloc(w->rec_len * sizeof(float));
        w->rec_lo_double = (double*)wtmalloc(w->rec_len * sizeof(double));
        w->rec_hi_double = (double*)wtmalloc(w->rec_len * sizeof(double));
