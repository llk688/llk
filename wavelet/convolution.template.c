    ...
    const size_t step, MODE mode)
{
    ...
    // 按照MODE指定的方式填充左边界，并计算小波系数
    // 最终实现不需要这个for循环内的处理（不做左填充），但需要确定好边界
    for (; i < F && i < N; i += step, ++o) {
        TYPE sum = 0;
        size_t j;
        for (j = 0; j <= i; ++j)
    ...
    // 如果数据长度小于滤波器长度，要做填充
    // 我们最终版本预期不会用此代码，因为要确保输入数据有足够长度完成一次卷积
    for (; i < F; i += step, ++o) {
        TYPE sum = 0;
        size_t j = 0;
    ...
    TYPE* const restrict output, const size_t O)
{
    ...
    // 这部分代码不用看，因为我们不使用周期填充(MODE_PERIODIZATION)
