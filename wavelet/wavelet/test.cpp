...
0.32736248,
    ...
    //Node<float>* root = new Node<float>("", NodeType::INTERMEDIATE, false, N,nullptr);
    //Node<float> root("", NodeType::INTERMEDIATE, false, N,nullptr);
    Node<float>* root = new Node<float>("", NodeType::INTERMEDIATE, false, N);
    ...
    for (size_t i = 0; i < input_data.size(); ++i) {
        ...
        updateCoefficients<float>(input_data[i], *root, wavelet, mode, levels);
        // 打印树结构以观察实时变化
        wpt.printTree(root);
        //   // printTree(root);
    }
