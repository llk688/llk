...
//WPT系数更新
template <typename T>
void updateCoefficients(float input_signal, Node<T>& root,const DiscreteWavelet* wavelet,MODE mode,int levels) {
    size_t N = wavelet->dec_len;
    int m = static_cast<int>(N);
    size_t step = 2; // 步长
    
    Node<T>* stack[20]; // 固定大小的栈levels层
    ...
    while (stack_top > 0) {
        // 出栈当前节点
        Node<T>* current_node = stack[--stack_top];
